...
class AuthDB
{
  ...
  function login(string $userId): void
  {
    ...
    $signature = hash_hmac('sha256', $sessionId, 'oui oui baguette');
  ...
  function register(array $user): void
  {
    ...
    $this->statementRegister->bindValue(':category', $user['category']);
  ...
  function isLoggedin(): array | false
  {
    ...
    if ($sessionId && $signature) {
      $hash = hash_hmac('sha256', $sessionId, 'oui oui baguette');
